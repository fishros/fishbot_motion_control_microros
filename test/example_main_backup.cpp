...
void microros_spin_task(void *param)
{
  while (true)
  {
    if (!rmw_uros_epoch_synchronized())
    {
      ...
      fishlog_debug("ros2", "current time :%ld", rmw_uros_epoch_millis());
      delay(100);
      continue;
    }
...
{
  preferences.clear();
  fishlog_debug("main", "first startup,init config to default value.\n");
  // 设置传输模式，默认SERIAL模式
...
{
  first_startup_setup();
  fishlog_debug("main", "config_name\t\tvalue\n-------------------------------------------------");
  fishlog_debug("main", "%s\t\t%d", CONFIG_NAME_IS_FIRST_STARTUP, preferences.getBool(CONFIG_NAME_IS_FIRST_STARTUP, false));
  fishlog_debug("main", "%s\t\t%d", CONFIG_NAME_TRANSPORT_MODE, preferences.getInt(CONFIG_NAME_TRANSPORT_MODE, CONFIG_DEFAULT_TRANSPORT_MODE));
  fishlog_debug("main", "%s\t\t%d", CONFIG_NAME_TRANSPORT_SERIAL_BAUD, preferences.getULong(CONFIG_NAME_TRANSPORT_SERIAL_BAUD, CONFIG_DEFAULT_TRANSPORT_SERIAL_BAUD));
  fishlog_debug("main", "%s\t\t%d", CONFIG_NAME_TRANSPORT_MODE_WIFI_SERVER_PORT, preferences.getULong(CONFIG_NAME_TRANSPORT_MODE_WIFI_SERVER_PORT, CONFIG_DEFAULT_TRANSPORT_MODE_WIFI_SERVER_PORT));
  fishlog_debug("main", "%s\t\t%s", CONFIG_NAME_WIFI_STA_SSID_NAME, preferences.getString(CONFIG_NAME_WIFI_STA_SSID_NAME, CONFIG_DEFAULT_WIFI_STA_SSID));
  fishlog_debug("main", "%s\t\t%s", CONFIG_NAME_WIFI_STA_PSWK_NAME, preferences.getString(CONFIG_NAME_WIFI_STA_PSWK_NAME, CONFIG_DEFAULT_WIFI_STA_PSWK));
  fishlog_debug("main", "%s\t\t%s", CONFIG_NAME_TRANSPORT_MODE_WIFI_SERVER_IP, preferences.getString(CONFIG_NAME_TRANSPORT_MODE_WIFI_SERVER_IP, CONFIG_DEFAULT_TRANSPORT_MODE_WIFI_SERVER_IP));
  fishlog_debug("main", "%s\t\t%s", CONFIG_NAME_ROS2_ODOM_FRAMEID_NAME, preferences.getString(CONFIG_NAME_ROS2_ODOM_FRAMEID_NAME, CONFIG_DEFAULT_ROS2_ODOM_TOPIC_NAME));
  fishlog_debug("main", "%s\t\t%s", CONFIG_NAME_ROS2_CMD_VEL_TOPIC_NAME, preferences.getString(CONFIG_NAME_ROS2_CMD_VEL_TOPIC_NAME, CONFIG_DEFAULT_ROS2_CMD_VEL_TOPIC_NAME));
  fishlog_debug("main", "%s\t\t%s", CONFIG_NAME_ROS2_ODOM_TOPIC_NAME, preferences.getString(CONFIG_NAME_ROS2_ODOM_TOPIC_NAME, CONFIG_DEFAULT_ROS2_ODOM_TOPIC_NAME));
  fishlog_debug("main", "%s\t\t%f", CONFIG_NAME_KINEMATIC_WHEEL_DISTANCE, preferences.getFloat(CONFIG_NAME_KINEMATIC_WHEEL_DISTANCE, CONFIG_DEFAULT_KINEMATIC_WHEEL_DISTANCE));
  fishlog_debug("main", "-------------------------------------------");
}
